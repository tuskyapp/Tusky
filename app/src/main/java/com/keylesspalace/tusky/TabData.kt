...
data class TabData(
    ...
    override fun hashCode() = Objects.hash(kind, arguments)
}

fun List<TabData>.hasTab(id: String): Boolean = this.find { it.kind == id } != null

fun createTabDataFromId(kind: String, arguments: List<String> = emptyList()): TabData {
    return when (kind) {
        HOME -> TabData(
            kind = HOME,
